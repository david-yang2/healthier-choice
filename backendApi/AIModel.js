...
export const submitAiRequestWithDb = async (base64EncodedImg) => {
  const harmfulIngredients = [];
  let parsedIngredients;

  // extract all ingredients from photo
  try {
    const allIngredients = await extractIngredients(base64EncodedImg);
    parsedIngredients = JSON.parse(allIngredients);
  } catch (err) {
    console.log(`There was an error extracting ingredients: ${err}`);
    throw err;
  }
  ...
  try {
    ...
    for (const ingredient of parsedIngredients) {
      // check if ingredient is in db already
      const ingredientInDb = await db.get(
      ...
      // add to harmfulIngredients list if there's a match and its harmful
      // 1 for true
      if (ingredientInDb && ingredientInDb.harmful === 1) {
        harmfulIngredients.push(ingredientInDb);
      }

      if (!ingredientInDb) {
        const searchResult = await searchIngredient(ingredient);
        const {
        ...
        await db.run(
          `INSERT INTO ingredients (ingredient, harmful, rating, description) VALUES (?, ?, ?, ?)`,
          [
            ingredientName
              .replace(/[^a-zA-Z0-9\s]/g, "")
              .trim()
    ...
    await db.close();
    console.log(`harmful ingredients list:`);
    console.log(harmfulIngredients);

    // return full ingredients list and array of harmful ingredients object
  } catch (err) {
    console.log(`this is mapping error ${err}`);
  }
};
