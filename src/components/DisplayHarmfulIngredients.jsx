...
const DisplayHarmfulIngredients = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      window.errorresult = parsedResult;
        ...
        <div className="rounded-xl border border-gray-200 bg-white shadow-sm ">
          <div className="px-4 py-3 bg-gray-50 border-b border-gray-200">
            <div className="text-sm font-semibold text-gray-700">
              All Ingredients
              <span className="ml-2 text-xs font-normal text-gray-500">({totalIngredients} detected)</span>
            </div>
          </div>
        ...
        <div className="rounded-xl border border-red-200 bg-white shadow-sm mb-10">
          <div className="px-4 py-3 bg-red-50 border-b border-red-200 flex items-center gap-2">
            ...
            <div className="text-sm font-semibold text-red-800">
              Harmful Ingredients
              <span className="ml-2 text-xs font-normal text-red-500">({harmful.length} flagged)</span>
            </div>
          </div>
            ...
            {harmful.map((item, idx) => (
              <div key={idx} className="px-4 py-4 flex flex-col gap-1">
                <div className="flex items-center justify-between">
                  <div className="text-sm font-semibold text-gray-900">{item.ingredient}</div>
  ...
  const displayContent = () => {
    if (harmfulIngredients) {
      return <div className="w-full max-w-4xl mx-auto h-full pr-1">{harmfulIngredientsContent()}</div>;
      ...
      return (
        <div className="w-full">
          <div className="rounded-xl border border-red-200 bg-white shadow-sm">
            <div className="px-4 py-3 bg-red-50 border-b border-red-200 flex items-center gap-2">
              <span className="text-red-500 text-base">⚠️</span>
              <div className="text-sm font-semibold text-red-800">Unable to Analyze Image</div>
            </div>
            <div className="px-4 py-4">
              <div className="text-sm text-gray-700 leading-relaxed">{errorProcessingImg.error}</div>
            </div>
          </div>
        </div>
      );
  ...
  window.results = harmfulIngredients;
  ...
  return (
    <div className="h-dvh flex flex-col">
      ...
      <button className="w-full text-lg m-0" onClick={() => setImageFile("")}>Take another photo</button>
    </div>
  );
};
...
const containerStyle = "flex flex-1 bg-gray-200 w-full px-3 py-10 overflow-y-auto";
